---
title: "MicrobiomeAnalysis_DADA2"
author: "Rebecca Valls and Yasmin Hilliam"
date: "Summer 2025"
...
These samples are from a mouse experiment. Stool was collected from two treatment groups of mice at three different time points. The two treatment groups of mice were treated with a gavage of (1) CF stool + PBS or (2) CF Stool + 3 Bacteroides strains. The tree time points were stool collected at (1) baseline, (2) during antibiotic treatment and (3) right after the gavage. 

Sequencing Information from SeqCenter: Samples were prepared using Zymo Research’s Quick-16S kit with phased primers targeting the V3/V4 regions of the 16S gene. Following clean up and normalization, samples were sequenced on a V3 MiSeq 622cyc flowcell to generate 2x301bp PE reads. 
# Libraries
Here we load the libraries needed to run the code
```{r}
#If running this from Discovery and submitting a job to process many samples, include this code so that the directory with your R packages is called on
#If you ever want to go this route, reach out to me and I can cp my packages to your R directory.
#.libPaths(c("/dartfs-hpc/rc/home/z/f003kcz/R/x86_64-pc-linux-gnu-library/4.2", $
#library("dada2")

#If you're doing this from your CPU, install and load these libraries:
...
pdf(file = "QualF.pdf",
    width = 10,
    height = 5)
plotQualityProfile(fnFs[1:5]) # Plot quality profiles for the first five forward reads
dev.off()
